...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    return check(node, nullptr, nullptr, less);
  }

  static bool check(const Node *node, const T *min, const T *max, Compare less) {
    if (!node) return true;

    if (min && !less(*min, node->datum)) return false;

    if (max && !less(node->datum, *max)) return false;

    return check(node->left, min, &node->datum, less) &&
          check(node->right, &node->datum, max, less);
  }
