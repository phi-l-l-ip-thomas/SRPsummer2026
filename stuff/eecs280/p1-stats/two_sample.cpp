...
pair<double, double> confidence_interval(vector<double> v, double width) {
  ...
  double lower = percentile(v, .5 - width/2);
  double upper = percentile(v, .5 + width/2);
  return {lower, upper};
}
